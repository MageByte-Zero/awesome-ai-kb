...
看操作复杂度是否是`O(N)`。[官方文档](https://redis.io/commands/)对每个命令的复杂度都有介绍，尽可能使用`O(1) 和 O(log N)`命令。

涉及到集合操作的复杂度一般为`O(N)`，比如集合**全量查询**`HGETALL、SMEMBERS`，以及集合的**聚合操作：**[SORT](https://redis.io/commands/sort)、[LREM](https://redis.io/commands/lrem)、 [SUNION](https://redis.io/commands/sunion)等。

> 有监控数据可以观测呢？代码不是我写的，不知道有没有人用了慢指令。

有两种方式可以排查到：

- 使用 Redis [慢日志功能](https://redis.io/commands/slowlog)查出慢命令；
...
![redis-pipeline](https://magebyte.oss-cn-shenzhen.aliyuncs.com/redis/20220220143454.png)
...
- 使用高效的命令代替。使用增量迭代的方式，避免一次查询大量数据，具体请查看[SCAN](https://redis.io/commands/scan)、[SSCAN](https://redis.io/commands/sscan)、[HSCAN](https://redis.io/commands/hscan)和[ZSCAN](https://redis.io/commands/zscan)命令。

除此之外，生产中禁用[KEYS 命令，它只适用于调试。](https://redis.io/commands/keys)因为它会遍历所有的键值对，所以操作延时高。

## Fork 生成 RDB 导致的延迟

[生成 RDB 快照](https://mp.weixin.qq.com/s/vpuMsen_s5Ye3Giz-59C3w)，Redis 必须 fork 后台进程。**fork 操作（在主线程中运行）本身会导致延迟。**
...
Redis 使用了 fork 生成 [RDB 做持久化提供了数据可靠性保证](https://mp.weixin.qq.com/s/vpuMsen_s5Ye3Giz-59C3w)。

当生成 RDB 快照的过程中，Redis 采用**[写时复制](https://mp.weixin.qq.com/s/vpuMsen_s5Ye3Giz-59C3w)**技术使得主线程依然可以接收客户端的写请求。
...
为了保证数据可靠性，[Redis 使用 AOF 和 RDB 快照实现当即快速恢复和持久化](https://mp.weixin.qq.com/s/vpuMsen_s5Ye3Giz-59C3w)。
