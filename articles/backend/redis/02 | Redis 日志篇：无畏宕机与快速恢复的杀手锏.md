...
上一篇[《Redis 核心篇：唯快不破的秘密》](https://mp.weixin.qq.com/s/8HN1PqqU57Kdz9ERwDY2cw)分析了 Redis 的核心数据结构、IO 模型、线程模型、根据不同数据使用合适的数据编码。深层次掌握真正快的原因！
...
![Redis 日志篇：无畏宕机与快速恢复的杀手锏](https://magebyte.oss-cn-shenzhen.aliyuncs.com/redis/20210204124323.png)
...
**Redis 系列篇章**围绕如下思维导图展开，这次一起探索 Redis 的高性能、持久化机制的秘密。
...
65 哥不错呀，开始考虑数据效率问题了。在[《Redis 核心篇：唯快不破的秘密》](https://mp.weixin.qq.com/s/8HN1PqqU57Kdz9ERwDY2cw)中我们知道他的单线程模型决定了我们要尽可能的避免会阻塞主线程的操作，避免 RDB 文件生成阻塞主线程。
...
[Tomcat 架构原理解析到设计借鉴](https://mp.weixin.qq.com/s/fU5Jj9tQvNTjRiT9grm6RA)

[从面试角度一文学完 kafka](https://mp.weixin.qq.com/s/QF9b9vpncLS24xMzFkanTQ)

[从 JMM 透析 volatile 与 synchronized 原理](https://mp.weixin.qq.com/s/6HKCnN2HDi84bwjElhZBcQ)

**关注「码哥字节」，每次都是炸裂硬核。阅读后如有收获请「点赞、分享、收藏」，谢谢支持.**

读者群已经开通，群里与各个大厂的大佬，不仅可以内推也可以学习。添加「码哥」个人微信，回复「加群」一起成长！

![码哥微信](https://magebyte.oss-cn-shenzhen.aliyuncs.com/juc/20201101164549.png)

**鸣谢**

redis 核心技术与实战: https://time.geekbang.org/column/intro/329
redis 深度历险：核心原理与应用实践: https://juejin.cn/book/6844733724618129422/section/6844733724714614797
redis 设计与实践: https://weread.qq.com/web/reader/d35323e0597db0d35bd957bk73532580243735b90b45ac8