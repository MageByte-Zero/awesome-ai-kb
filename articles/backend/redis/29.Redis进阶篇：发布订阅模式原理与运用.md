...
Redis 通过 [`SUBSCRIBE`](https://redis.io/commands/subscribe)，[`UNSUBSCRIBE`](https://redis.io/commands/unsubscribe)和[`PUBLISH`](https://redis.io/commands/publish) 实现发布订阅消息传递模式，Redis 提供了两种模式实现，分别是「发布/订阅到频道」和「发布\订阅到模式」。
...
使用 [UNSUBSCRIBE](http://redis.readthedocs.org/en/latest/pub_sub/unsubscribe.html#unsubscribe) 命令可以退订指定的「模式」不会影响通过 `subscribe 命令订阅的频道。
...
官方文档：https://github.com/redisson/redisson/wiki/6.-distributed-objects/#67-topic
...
1. `listSearchKey(c->pubsub_patterns,pattern)`：根据 pattern 从 redisServer.pubsub_patterns 字典查找是否已经存在该模式的 key，存在则调用`addReplyPubsubPatSubscribed` 通知客户端已经订阅过了，否则继续执行以下逻辑。
...
哨兵集群中，每个哨兵节点利用 Pub/Sub 发布订阅实现哨兵之间的相互发现彼此和找到 Slave，详情点击 ->[《哨兵集群原理那些事》](https://mp.weixin.qq.com/s/m3j2WZdFas8fjLRsykGcBQ)。

哨兵与 Master 建立通信后，利用 master 提供发布/订阅机制在`__sentinel__:hello`发布自己的信息，比如身高体重、是否单身、IP、端口……，同时订阅这个频道来获取其他哨兵的信息，就这样实现哨兵间通信。

**消息队列**

之前「码哥」跟大家分享过如何利用 [Redis List](https://mp.weixin.qq.com/s/Pv7IXUQQnhGCGMM9n0VfkA) 与 [Stream 实现消息队列](https://mp.weixin.qq.com/s/FC7n8KVQVejRzhjwlq0PMQ)。
